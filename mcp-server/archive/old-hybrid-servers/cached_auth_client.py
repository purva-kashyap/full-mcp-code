"""
Client with token caching - authenticate once, reuse token
"""
import asyncio
import json
import webbrowser
...
async def main():
    """Test token caching - only authenticate if needed"""
    ...
    async with Client("http://localhost:8001/mcp") as client:
        ...
        user_email = input("Enter your email (or press Enter for new auth): ").strip()
        
        access_token = None
        
        # Try to get cached token first
        if user_email:
            print(f"\n🔍 Checking for cached token for {user_email}...")
            cache_result = await client.call_tool("get_cached_token", {
                "user_id": user_email
            })
            
            cache_content = cache_result.content[0].text
            if isinstance(cache_content, str):
                cache_data = json.loads(cache_content)
            else:
                cache_data = cache_content
            
            if cache_data.get("cached"):
                access_token = cache_data["access_token"]
                print(f"✅ Using cached token (expires: {cache_data['expires_at']})")
                print("   No authentication needed! 🎉\n")
            else:
                print("   No cached token found, need to authenticate.\n")
        ...
        if not access_token:
            ...
            user_email = token_data.get("user_id", "unknown")
            
            print("✅ Access token obtained and cached!\n")
            print(f"💾 Token cached for: {user_email}")
            print("   Next time, just provide this email to skip authentication!\n")
        ...
        if "error" not in profile:
            ...
            print("="*50)
            print("\n✨ Token is cached - run again with same email to skip auth!")
        else:
            print(f"❌ Failed to get profile: {profile.get('error')}")


if __name__ == "__main__":
    print("🚀 Hybrid MCP Server - Token Caching Demo\n")
    print("Features:")
    print("  ✅ Authenticate once, token cached locally")
    print("  ✅ Auto-refresh when token expires")
