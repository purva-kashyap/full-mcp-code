...
def request(
    ...
    print(f"[GRAPH] Getting token for account: {account_id}", file=sys.stderr)
    print(f"[GRAPH] Token length: {len(token)}", file=sys.stderr)
    print(f"[GRAPH] Token prefix: {token[:20]}...", file=sys.stderr)
    ...
    while retry_count <= max_retries:
        try:
            url = f"{BASE_URL}{path}"
            print(f"[GRAPH] Making request: {method} {url}", file=sys.stderr)
            print(f"[GRAPH] Headers: {headers}", file=sys.stderr)
            print(f"[GRAPH] Params: {params}", file=sys.stderr)
            ...
            print(f"[GRAPH] Response status: {response.status_code}", file=sys.stderr)
            print(f"[GRAPH] Response headers: {dict(response.headers)}", file=sys.stderr)
            if response.status_code >= 400:
                print(f"[GRAPH] Response body: {response.text[:500]}", file=sys.stderr)
