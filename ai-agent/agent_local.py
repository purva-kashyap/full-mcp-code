...
class LocalLLMProvider:
    ...
    def __init__(self, model: str = "llama-3.3-70b-instruct", base_url: str = "http://localhost:11434"):
        ...
        console.print(f"[dim]   API endpoint: {self.api_url}[/dim]")
    
    async def complete(self, messages: List[Message], tools: List[Dict]) -> Dict[str, Any]:
        """Generate completion using local LLM with function calling simulation"""
        
        # Convert messages to format for local LLM
        formatted_messages = []
        system_message = None
        ...
        for msg in messages:
            if msg.role == "system":
                ...
                formatted_messages.append({
                    "role": "assistant",
                    "content": msg.content if msg.content else "I'll use the following tools:"
                })
            else:
                formatted_messages.append({
                    "role": msg.role,
